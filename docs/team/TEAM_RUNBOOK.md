...
> Day-to-day playbook for operating the 26-agent team (24 specialists + 2 verifiers). Read this before every session until you've internalized the patterns.
...
**Fix:** All 26 agent prompts should have this section (validated by the contract test suite). If missing, run `python3 .claude/tests/agents/run_contract_tests.py --agent <name>` to identify the gap. To recover mid-session, SendMessage to the agent reminding them to use SendMessage.
...
**A:** No. The Shadow Mind is optional. The 26-agent team operates fully without it. Enable it if you want probabilistic pattern-based guidance via `[NEXUS:INTUIT]`; skip it if you don't.
...
# Team continues operating — 286/286 contract tests still pass
```
...
- All 286/286 contract tests pass whether Shadow Mind is enabled or disabled
...
*Last updated: 2026-04-18 — v3.0 with 26 agents, hiring pipeline operational notes, and Shadow Mind operational procedures.*
